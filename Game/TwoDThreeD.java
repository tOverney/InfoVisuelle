...
public class TwoDThreeD {
	...
	private PVector rotationFromMatrix(float[][]  mat) {
		...
		rot.x = (float)Math.atan2(mat[2][1]/Math.cos(rot.y),
			mat[2][2]/Math.cos(rot.y));
		rot.z = (float)Math.atan2(mat[1][0]/Math.cos(rot.y),
			mat[0][0]/Math.cos(rot.y));

		return rot;
	}

	static class CWComparator implements Comparator<PVector> { PVector center;
		...
		public int compare(PVector b, PVector d) {
			if(Math.atan2(b.y-center.y,b.x-center.x) < 
	...
	public static List<PVector> sortCorners(List<PVector> quad){
        ...
        for (int i = 0; i <= 3; i++) {
        	double currentDist = quad.get(i).dist(origin);
        ...
        Collections.rotate(quad, indexClosest);
